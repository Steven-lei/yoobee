...
resource "aws_security_group" "web_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    security_groups = [aws_security_group.alb_sg.id]  #from loadbalancer 
  }
  ingress {
    description = "Allow HTTP"
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
    security_groups = [aws_security_group.openvpn_sg.id]  #from openvpn 
  }
  # ingress {
  #   description = "Allow HTTP"
  #   from_port   = 80
  #   to_port     = 80
  #   protocol    = "tcp"
  #   cidr_blocks = ["0.0.0.0/0"]   #from vpnsubnet 
  # }
  # ingress {
...
#     accessing wordpress page: http://${aws_instance.testwebserver.public_ip}/wordpress/
#     EOT
# }
